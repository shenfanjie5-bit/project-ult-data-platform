...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE nsp.nspname = 'data_platform'
          AND typ.typname = 'candidate_payload_type'
    ) THEN
    ...
    IF NOT EXISTS (
        ...
        JOIN pg_namespace nsp ON nsp.oid = typ.typnamespace
        WHERE nsp.nspname = 'data_platform'
          AND typ.typname = 'validation_status'
    ) THEN
...
CREATE TABLE IF NOT EXISTS data_platform.candidate_queue (
    ...
    CONSTRAINT candidate_queue_payload_no_system_fields CHECK (
        jsonb_typeof(payload) = 'object'
        AND NOT (payload ? 'submitted_at')
